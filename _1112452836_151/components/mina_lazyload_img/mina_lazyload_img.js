var i = "data:image/png;base64,", t = require("../../npm/@dp/logan-wxapp/build/wxlogan.js");

Component({
    properties: {
        imgSrc: {
            type: String,
            value: i
        },
        imgMode: {
            type: String,
            value: "scaleToFill"
        },
        imgClass: {
            type: String,
            value: ""
        }
    },
    data: {
        minaEnv: "wx",
        show: !1,
        default: i
    },
    created: function() {
        this.enableSetData = !0;
        var i = wx.getSystemInfoSync();
        this.viewPortWidth = i.windowWidth, this.viewPortHeight = i.windowHeight;
    },
    methods: {
        judge: function(i) {
            var t = this;
            !this.data.show && this.enableSetData && (this.enableSetData = !1, wx.createSelectorQuery().in(this).select(".img-class").boundingClientRect(function(i) {
                i && i.top - 100 <= t.viewPortHeight && i.left - 80 <= t.viewPortWidth ? (console.log("ready to show image --------\x3e>>>>"), 
                t.setData({
                    show: !0
                }, function() {
                    t.enableSetData = !0;
                }), setTimeout(function() {
                    t.setData({
                        useTransition: !0
                    });
                }, 1)) : t.enableSetData = !0;
            }).exec());
        },
        bindLoad: function(i) {
            this.data.show && this.triggerEvent("lazyloadsuccess", i);
        },
        bindError: function(i) {
            var e = this;
            t.log("[lazyload][" + new Date().toLocaleTimeString() + "]--\x3e【img onError】"), 
            this.data.show && (this.triggerEvent("lazyloaderror", i), this.setData({
                show: !1
            }, function() {
                e.enableSetData = !1;
            }));
        }
    }
});